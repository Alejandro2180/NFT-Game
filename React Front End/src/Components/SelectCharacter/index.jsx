...
const SelectCharacter = ({ setCharacterNFT }) => {
  ...
  const mintHeroNFTAction = async (characterId) => {
    try{
      if (gameContract) {
        setMintingCharacter(true);
        console.log("Minting character in progress...");
        const mintTxn = await gameContract.mintHeroNFT(characterId);
        await mintTxn.wait();
  ...
  useEffect(() => {
    ...
    if(gameContract){
      getCharacters();
      gameContract.on('HeroNFTMinted', onCharacterMint);
    }

    return () => {
      if(gameContract){
        gameContract.off('HeroNFTMinted', onCharacterMint);
      }
    };
  }, [gameContract]);
    ...
    characters.map((character, index) => (
      <div className="character-item" key={character.name}>
        ...
        <button
          ...
          onClick={()=> mintHeroNFTAction(index)}
        >{`Mint ${character.name}`}
        </button>
      </div>
    ));
